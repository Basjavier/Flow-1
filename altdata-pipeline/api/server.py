...
def get_high_urgency():
    """Solo señales HIGH urgency — para alertas."""
    signals = db.get_active_signals(
        min_confidence=0.75,
        urgency="HIGH",
        hours=24,
...
def get_re_stats(comuna: str):
    """Estadísticas mercado inmobiliario por comuna."""
    stats = db.get_re_stats(comuna)
    if stats["n"] == 0:
        raise HTTPException(
            status_code=404,
            detail=f"Sin datos para {comuna}"
        )
    return {
        "comuna": comuna,
        "stats": stats,
...
def pipeline_status():
    """Estado de últimas ejecuciones del pipeline."""
...
@app.get("/cost")
def cost_estimate():
    """Estimado de tokens usados y costo."""
    total_signals = db.conn.execute(
        "SELECT COUNT(*) FROM signals"
    ).fetchone()[0]
    total_hechos = db.conn.execute(
        "SELECT COUNT(*) FROM hechos_esenciales"
    ).fetchone()[0]

    # Estimado conservador: 850 tokens/llamada
    tokens_used = total_signals * 850
    cost_usd = tokens_used / 1_000_000 * 3  # Sonnet rate
