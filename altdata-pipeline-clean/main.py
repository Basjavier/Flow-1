...
def _bootstrap_schema():
    """Crea archivo DuckDB + tablas si no existen. Hay que correrlo ANTES
    de abrir cualquier conexion read-only desde la API."""
...
async def run_tests():
    """Test rapido de todas las conexiones."""
    ...
    try:
        ...
        results["bloomberg"] = f"WARN {e} (requiere Terminal abierto)"
...
if __name__ == "__main__":
    ...
    if "--test" in args:
        ...
        # Aseguramos que el archivo + schema existan antes de que la API
        # lo abra read-only.
        _bootstrap_schema()
        run_api()
    elif "--once" in args:
        ...
        # Modo completo: bootstrap schema, levantar API (read-only) en
        # thread, luego correr pipeline (writer) en el main thread.
        _bootstrap_schema()
        import threading
