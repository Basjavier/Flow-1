...
class Database:
    def __init__(self, read_only: bool = False):
        ...
        if not read_only:
            ...
            # DUCKDB_PATH pueda apuntar a cualquier ruta (ej: /tmp/altdata/).
            Path(DUCKDB_PATH).parent.mkdir(parents=True, exist_ok=True)
        self.conn = duckdb.connect(DUCKDB_PATH, read_only=read_only)
        if not read_only:
            self._create_tables()
        mode = "read-only" if read_only else "read-write"
        logger.info(f"DuckDB conectado: {DUCKDB_PATH} ({mode})")
    ...
    def get_active_signals(
        ...
        # Whitelist urgency (defensa en profundidad - tambien se bindea abajo)
        if urgency is not None and urgency not in ("HIGH", "MEDIUM", "LOW"):
            urgency = None
        # DuckDB no soporta binding dentro de INTERVAL: casteamos hours a int.
        hours = int(hours)
        query = f"""
